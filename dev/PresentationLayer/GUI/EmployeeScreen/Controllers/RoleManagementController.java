...
public class RoleManagementController {
                                    ...
                                    try {
                                        ...
                                        if (idIndex >= 0) {
                                            ...
                                            if (endIndex > startIndex) {
                                                employeeId = Long
                                                        .parseLong(employeeJson.substring(startIndex, endIndex).trim());
                                            }
                                        }
                                        ...
                                        if (firstNameIndex >= 0) {
                                            ...
                                            if (endIndex > startIndex) {
                                                ...
                                                if (lastNameIndex >= 0) {
                                                    int lastNameStartIndex = lastNameIndex + 12; // Length of
                                                                                                 // "\"lastName\":"
                                                    int lastNameEndIndex = employeeJson.indexOf("\"",
                                                            lastNameStartIndex + 1);
                                                    if (lastNameEndIndex > lastNameStartIndex) {
                                                        String lastName = employeeJson.substring(lastNameStartIndex + 1,
                                                                lastNameEndIndex);
                                        ...
                                        if (activeIndex >= 0) {
                                            ...
                                            if (endIndex > startIndex) {
                                                active = Boolean.parseBoolean(
                                                        employeeJson.substring(startIndex, endIndex).trim());
                                            }
                                        }
    ...
    /**
     * Adds a permission to the list based on whether it exists in the permissions
     * set.
     * 
     ...
     */
    private void addPermission(Set<String> permissions, String permissionName, List<PermissionEntry> permissionsList,
            String description) {
     ...
     */
    private void addDefaultPermission(List<PermissionEntry> permissionsList, String permissionName, String description,
            boolean granted) {
        permissionsList.add(new PermissionEntry(
                permissionName,
                description,
    ...
    public void showCreateRoleDialog() {
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == createButtonType) {
                ...
                // Employee permissions
                if (viewEmployeeCheck.isSelected())
                    permissions.add("VIEW_EMPLOYEE");
                if (createEmployeeCheck.isSelected())
                    permissions.add("CREATE_EMPLOYEE");
                if (editEmployeeCheck.isSelected())
                    permissions.add("EDIT_EMPLOYEE");
                if (deleteEmployeeCheck.isSelected())
                    permissions.add("DELETE_EMPLOYEE");
                if (adminEmployeeCheck.isSelected())
                    permissions.add("ADMIN_EMPLOYEE");

                // Shift permissions
                if (viewShiftCheck.isSelected())
                    permissions.add("VIEW_SHIFT");
                if (createShiftCheck.isSelected())
                    permissions.add("CREATE_SHIFT");
                if (editShiftCheck.isSelected())
                    permissions.add("EDIT_SHIFT");
                if (deleteShiftCheck.isSelected())
                    permissions.add("DELETE_SHIFT");
                if (adminShiftCheck.isSelected())
                    permissions.add("ADMIN_SHIFT");

                // System permissions
                if (viewSystemCheck.isSelected())
                    permissions.add("VIEW_SYSTEM");
                if (editSettingsCheck.isSelected())
                    permissions.add("EDIT_SETTINGS");
                if (manageRolesCheck.isSelected())
                    permissions.add("MANAGE_ROLES");
                if (viewLogsCheck.isSelected())
                    permissions.add("VIEW_LOGS");
                if (adminSystemCheck.isSelected())
        ...
        result.ifPresent(roleData -> {
            ...
            if (employeeService != null) {
                ...
                addPermission(permissions, "ADMIN_EMPLOYEE", employeePermissions,
                        "Full administrative access to employees");
                ...
                if (employeeService != null) {
                    try {
                        ...
                        for (String permission : permissionsToRemove) {
                            String removeResult = employeeService.removePermissionFromRole(doneBy, roleName,
                                    permission);
                        ...
                        if (hasChanges) {
                            ...
                            // Process employee permissions
                            addPermission(updatedPermissions, "VIEW_EMPLOYEE", employeePermissions,
                                    "View employee details");
                            addPermission(updatedPermissions, "CREATE_EMPLOYEE", employeePermissions,
                                    "Create new employees");
                            addPermission(updatedPermissions, "EDIT_EMPLOYEE", employeePermissions,
                                    "Edit employee information");
                            addPermission(updatedPermissions, "DELETE_EMPLOYEE", employeePermissions,
                                    "Delete employees");
                            addPermission(updatedPermissions, "ADMIN_EMPLOYEE", employeePermissions,
                                    "Full administrative access to employees");
                            ...
                            addPermission(updatedPermissions, "ADMIN_SHIFT", shiftPermissions,
                                    "Full administrative access to shifts");

                            // Process system permissions
                            addPermission(updatedPermissions, "VIEW_SYSTEM", systemPermissions,
                                    "View system information");
                            addPermission(updatedPermissions, "EDIT_SETTINGS", systemPermissions,
                                    "Edit system settings");
                            addPermission(updatedPermissions, "MANAGE_ROLES", systemPermissions,
                                    "Manage roles and permissions");
                            addPermission(updatedPermissions, "VIEW_LOGS", systemPermissions, "View system logs");
                            addPermission(updatedPermissions, "ADMIN_SYSTEM", systemPermissions,
                    ...
                    addPermission(permissions, "MANAGE_ROLES", systemPermissions, "Manage roles and permissions");
                    addPermission(permissions, "VIEW_LOGS", systemPermissions, "View system logs");
                    addPermission(permissions, "ADMIN_SYSTEM", systemPermissions,
                            "Full administrative access to system");
    ...
    private static class RoleData {
        ...
        public RoleData(String name, String description,
            ...
            this.employeePermissions = employeePermissions != null ? new ArrayList<>(employeePermissions)
                    : new ArrayList<>();
            ...
            if (employeePermissions != null)
                this.allPermissions.addAll(employeePermissions);
            if (shiftPermissions != null)
                this.allPermissions.addAll(shiftPermissions);
            if (systemPermissions != null)
                this.allPermissions.addAll(systemPermissions);
        ...
        public List<PermissionEntry> getAllPermissions() {
            return allPermissions;
        }

        public List<PermissionEntry> getEmployeePermissions() {
            return employeePermissions;
        }

        public List<PermissionEntry> getShiftPermissions() {
            return shiftPermissions;
        }

        public List<PermissionEntry> getSystemPermissions() {
            return systemPermissions;
        }

        public List<EmployeeWithRole> getEmployees() {
            return employees;
        }
    }
