...
public class EmployeeDetailsController {
    ...
    private void loadShifts() {
        ...
        ObservableList<ShiftEntry> shiftData = FXCollections.observableArrayList(
            new ShiftEntry(LocalDate.now().plusDays(1), "Morning", "08:00 - 16:00", "Branch " + employee.getBranchId(), "Cashier"),
            new ShiftEntry(LocalDate.now().plusDays(3), "Evening", "16:00 - 00:00", "Branch " + employee.getBranchId(), "Manager"),
            new ShiftEntry(LocalDate.now().plusDays(5), "Morning", "08:00 - 16:00", "Branch " + employee.getBranchId(), "Stocker"),
            new ShiftEntry(LocalDate.now().plusDays(7), "Evening", "16:00 - 00:00", "Branch " + employee.getBranchId(), "Cashier")
        );
    ...
    private void loadAvailability() {
        ...
        for (int col = 1; col <= 7; col++) {
            ...
            for (int row = 1; row <= 3; row++) {
                String shift;
                if (row == 1) shift = "MORNING";
                else if (row == 2) shift = "EVENING";
                else shift = "NIGHT";
    ...
    public void toggleEmployeeStatus() {
        // In a real implementation, this would update the employee's status in the service layer
